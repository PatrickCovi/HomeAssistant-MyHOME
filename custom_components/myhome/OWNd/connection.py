...
from .message import OWNMessage, OWNSignaling, OWNHeatingEvent, OWNLightingCommand
                ...
                if isinstance(resulting_message, OWNSignaling):
                    ...
                    if resulting_message.is_nack():
                        if trial_number > 2:
                            ...
                            self._logger.error(
                                "%s Could not send message `%s`. Retrying (%d)...", self._gateway.log_id, message, trial_number
                            )
